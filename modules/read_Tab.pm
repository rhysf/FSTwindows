...
sub save_column_to_line_hash {
	my ($file, $column) = @_;

	# Save from file
	my %interest;
	my $line_count = 0;
	warn "save_columns_to_line_hash: saving $column column -> line from $file...\n";
	open my $fh, '<', $file or die "Cannot open $file: $!\n";
	LIST: while(my $line=<$fh>) {
		chomp $line;
		next LIST if($line =~ m/^\n/);
		...
		my @bits = split /\t/, $line;
		die "Cannot find column $column in line $line\n" if(!defined $bits[$column]);
		$interest{$bits[$column]} = $line;
		$line_count++;
	}
	close $fh;
	warn "$line_count lines in $file\n";
